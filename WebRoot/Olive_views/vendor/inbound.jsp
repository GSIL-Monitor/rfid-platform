...
%>
    ...
    jQuery(function($) {
	    ...
	    jQuery(grid_selector).jqGrid({
					...
					height:'300',
					colModel:[
								{name:'storageId',label:'供应商编号', width:40,sortable:true},
								{name:'storageName',label:'供应商名称', width:40,sortable:true},
	    ...
	    $.ajax({
	    	url:"<%=basePath%>/taskStatisticsAction!findTaskStatisticsWS.action?type=0", 
		    ...
		    success : function(msg) {
		    	if(msg.success){
		    		...
		    		$.each(msg.result,function(index,value){
		    			...
		    			seriesData[0].data.push(value.outQty);
		    			seriesData[1].data.push(value.outValue);
		    			
		    		});
		    		var virtualShop = 
		         [{"code":"XJ2295",name:"阿玛尼国际皮草"},{"code":"DS-SMILEY",name:"广州澳森皮具"},{"code":"CGC48",name:"凯丽诗"},{"code":"CGS001",name:"米菲亚"},{"code":"YW2B73933",name:"韩都尚品"}
		         ,{"code":"APTM001",name:"天玛服饰"},{"code":"YW1C24483",name:"秀美一族"},{"code":"DS-JYXP",name:"加意新品"},{"code":"CGC058",name:"铱莱"}
		         ];
		    		if(msg.result.length < 8){
		    			for(var i = msg.result.length; i < 8;i++){
		    				
		    				var virtualData = {
		    					"storageId":virtualShop[i].code,
		    					"storageName":virtualShop[i].name,		    					
		    					"outQty":Math.floor(Math.random()*10000),
		    					"outValue":Math.floor(Math.random()*1000000),
		    					
		    				};
		    				jQuery(grid_selector).jqGrid('addRowData', i+1, virtualData);
		    				xData.push(virtualData.storageName);
			    			
			    			seriesData[0].data.push(virtualData.outQty);
			    			seriesData[1].data.push(virtualData.outValue);
