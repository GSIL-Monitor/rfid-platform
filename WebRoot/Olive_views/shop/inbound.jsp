...
%>
    ...
    jQuery(function($) {
	    ...
	    jQuery(grid_selector).jqGrid({
					...
					height:'300',
					colModel:[
								{name:'storageId',label:'门店编号', width:40,sortable:true},
								{name:'storageName',label:'门店名称', width:40,sortable:true},								
								...
								{name:'inventoryQty',label:'盘点数量', width:40},
								{name:'inventoryValue',label:'盘点金额', width:40},
	    ...
	    $.ajax({
	    	url:"<%=basePath%>/taskStatisticsAction!findTaskStatisticsWS.action?type=4", 
		    ...
		    success : function(msg) {
		    	if(msg.success){
		    		...
		    		$.each(msg.result,function(index,value){
		    			...
		    			seriesData[4].data.push(value.inventoryQty);
		    			seriesData[5].data.push(value.inventoryValue);
		    			seriesData[6].data.push(value.stockQty);
		    			seriesData[7].data.push(value.stockValue);
		    			
		    		});
		    		var virtualShop = [{"code":"BJ0001",name:"北京三里屯店"},{"code":"SH0001",name:"上海人民广场店"},{"code":"WX0001",name:"无锡三阳广场店"},{"code":"XA0001",name:"西安曲江银泰店"},{"code":"GZ0001",name:"广州万达广场店"}];
		    		if(msg.result.length < 5){
		    			for(var i = msg.result.length; i < 5;i++){
		    				...
		    				var virtualData = {
		    					...
		    					"inventoryQty":Math.floor(Math.random()*(1000000-100000+1)+100000),
		    					"inventoryValue":Math.floor(Math.random()*(100000000-10000000+1)+10000000),
			    			...
			    			seriesData[4].data.push(virtualData.inventoryQty);
			    			seriesData[5].data.push(virtualData.inventoryValue);
			    			seriesData[6].data.push(virtualData.stockQty);
			    			seriesData[7].data.push(virtualData.stockValue);
    ...
    function initshopLineChart(xData,seriesData) {
        ...
        var shopLineChartOptions =  {
        		 tooltip : {
        		    ...
        		    legend: {
        		        data:['入库数量','入库金额','出库数量','出库金额','盘点数量','盘点金额','库存数量','库存金额']
        		    },
        		    grid: {
        		        left: '3%',
        		        right: '4%',
