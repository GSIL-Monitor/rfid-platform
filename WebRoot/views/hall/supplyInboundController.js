...
function resetData() {
    var gridData = $("#supplyGrid").data("kendoGrid");
...
function iniSupplyGrid() {
    $("#supplyGrid").kendoGrid({
        toolbar: ["excel"],
        excel: {
            allPages:true,
            fileName: "样衣调拨统计.xlsx",
            proxyURL: basePath + "/hall/supplyInbound/export.do",
            filterable: true
        },
        ...
        dataSource: {
            schema : {
                ...
                model : {
                    fields: {
                        ...
                        scanDate: {type: "date"},   // 任务日期
                        deviceId: {type: "string"}, // 扫描设备
                        qty:{type:"number"},        // 数量
                        ownerId: {type: "string"},  // 入库方编号
                        unitName: {type: "string"}, //入库方名称
                        group:{type:"string"},      // 批次
            ...
            transport: {
                read: {
                    url: basePath + "/hall/supplyInbound/list.do",
                    type:"POST",
                    dataType: "json",
        ...
        columns: [
            ...
            {field:"unitName",title:"入库方名称",width:150,sortable:false,groupable:false,filterable:false},
            ...
            {field:"",title:"批次",width:100,sortable:false,groupable:false,filterable:false,
                template:function (data) {
                    var code=data.code;
                    var styleId=data.styleId;
                    return code.substring(styleId.length,code.length);
                }
            },
            ...
            {field:"remark",title:"备注",width:300,sortable:false,groupable:false,filtable:false}
        ]
    });
}
