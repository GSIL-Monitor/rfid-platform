                        ...
                        <div class="widget-main padding-12">
                            <form class="form-horizontal" role="form">
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-2">
                                        <input class="form-control" id="search_token" name="filter_LIKES_token"
                                               type="text" disabled
                                               value='<c:choose><c:when test="${business.token==8}">采购入库</c:when><c:when test="${business.token==23}">退货入库</c:when><c:when test="${business.token==25}">调拨入库</c:when><c:when test="${business.token==29}">调整入库</c:when></c:choose>'/>
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <label class="col-xs-1 control-label" for="search_origId">入库仓库</label>

                                    <div class="col-xs-2">
                                        <input class="form-control" id="search_origId" name="origId"
                                               type="text" disabled value="[${business.destId}]${business.destName}"/>
                                    </div>
                                    <label class="col-xs-1 control-label" for="search_origName">发货方</label>

                                    <div class="col-xs-2">
                                        <input class="form-control" id="search_origUnitName"
                                               name="filter_LIKES_origUnitName"
                                               type="text" disabled
                                               value="[${business.origUnitId}]${business.origUnitName}"/>
                                    </div>
                                    <label class="col-xs-1 control-label" for="search_origName">发货仓库</label>

                                    <div class="col-xs-2">
                                        <input class="form-control" id="search_origName" name="filter_LIKES_origName"
...
<script type="text/javascript">
    ...
    function initGrid() {
        ...
        $("#detailgrid").jqGrid({
            height: 500,
            url: basePath + "/task/inbound/detailPage.do?id=" + id,
            datatype: "json",
            type:"POST",
            colModel: [
    ...
    function initRecordGrid() {
        ...
        $("#recordgrid").jqGrid({
            height: 500,
            url: basePath + "/task/inbound/recordPage.do?id=" + id,
            datatype: "json",
            type:'POST',
            colModel: [
                {name: 'id', label: 'id', hidden: true, width: 40},
