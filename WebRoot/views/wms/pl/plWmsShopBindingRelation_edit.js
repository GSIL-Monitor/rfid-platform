...
function initKendoUI(){
    var dataSource = new kendo.data.TreeListDataSource({
        transport: {
            read: {
                url: basePath + "/wms/pl/rackBind/list.do",
                type: "POST",
                dataType: "json",
    ...
    $("#treelist").kendoTreeList({
        ...
        change: function() {
            ...
            $.ajax({
                type: "POST",
                url:basePath+"/wms/pl/rackBind/detail.do?id="+data.id+"&type="+data.type,
                dataType: "json",
...
//根据 选择的仓店 初始化区下拉框
    $.ajax({
        type: "POST",
        url: basePath + "/wms/pl/rack/findFaByShopId.do?shopId=" + shopId,
        dataType: "json",
        success: function (result) {
...
function initShopIdSelect(){
    ...
    $.ajax({
        type: "POST",
        url: basePath +"/sys/warehouse/list.do?filter_INI_type=4",
        dataType: "json",
        success: function (result) {
            var json = result;
...
function selectFloorAreaId(){
    ...
    initRaSelect(floorAreaId);
}
function initRaSelect(floorAreaId) {
//根据 选择的区 初始化库位下拉框
    $.ajax({
        type: "POST",
        url: basePath + "/wms/pl/rack/findRaByFaId.do?floorAreaId=" + floorAreaId,
        dataType: "json",
        success: function (result) {
            var json = result.result;
            $("#form_rackId").append("<option>-请选择-</option>");
            for (var i = 0; i < json.length; i++) {
...
function  bind(){
    ...
    $.ajax({
        type: "POST",
        data:formData,
        url: basePath + "/wms/pl/rackBind/bindShop.do",
        dataType: "json",
        success: function (result) {
...
function unbind() {
    ...
    if (data) {
        console.log(data.id);
        $.ajax({
            url: basePath + "/wms/pl/rackBind/unbind.do?id=" + data.id,
            type: "post",
            dataType: "json",
            success: function (result) {
                if (result.success == true || result.success == 'true') {
