...
<script>
    $(function () {
        if ("${rack.id}" != "") {
            ...
            $("#form_parentName").val("${rack.parentName}");
...
//                        },
                        ...
                        remote: {//ajax验证。server result:{"valid",true or false} 向服务发送当前input name值，获得一个json数据。例表示正确：{"valid",true}
                            url: basePath + "/wms/pl/rack/checkRackBarcode.do?id="+$("#form_id").val()+"&parentId="+$("#form_parentId").val(),//验证地址
                            message: '条码已存在',//提示消息
    ...
    function save(){
        ...
        $.ajax({
            type: "POST",
            url: basePath + "/wms/pl/rack/saveRa.do",
            data: formData,
            dataType: "json",
    ...
    function saveImg(barcode){
        ...
        xhr.open("POST", basePath + "/wms/pl/rack/saveImg.do?barcode="+barcode+"&flag=ra",false);
        xhr.send(fd);
    }
</script>
