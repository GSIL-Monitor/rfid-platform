...
<div class="main-container" id="main-container">
    ...
    <div class="main-content">
        <div class="main-content-inner">
            ...
            <div id="page-content">
                <div class="col-lg-12">
                    <form class="form-horizontal" role="form" id="rackEditForm" style="margin-top: 18px">
...
<script>
    $(function () {
        if ("${rack.id}" != "") {
            $("#form_name").val("${rack.name}");
            $("#form_deviceId").val("${rack.deviceId}");
            $("#form_parentId").val("${rack.parentId}");
            $("#form_barcode").val("${rack.barcode}");
            if("${rack.sales}"=="false"){
                //$("#form_sales").find("option[value='0']").attr("selected",true);
                var html =' <input id="form_sales" class="form-control" name="sales" type="text" value="${rack.sales}" readonly/>';
                $("#sales").html(html);
            }else{
                $("#form_sales").find("option[value='1']").attr("selected",true);
            }
            $("#form_remark").val("${rack.remark}");
            $("#form_image").val("${rack.image}");
            $("#form_id").val("${rack.id}");
            initRackFormValid(false,"${rack.barcode}","${shopId}");
        }
        if ("${parentId}" != "") {
            $("#form_parentId").val("${parentId}");
           // $("#form_sales").val("${sales}");
            if("${sales}"=="false"){
    ...
    function initRackFormValid(isAdd, oldBarcode,shopId) {
        $('#rackEditForm').bootstrapValidator({
            message: '输入值无效',
            feedbackIcons: {
                ...
                barcode: {
                    validators: {
                        ...
                        remote: {//ajax验证。server result:{"valid",true or false} 向服务发送当前input name值，获得一个json数据。例表示正确：{"valid",true}
                            url: basePath + "/wms/pl/checkBarcode.do?flag=ra&isAdd=" + isAdd + "&oldBarcode=" + oldBarcode+"&shopId="+shopId,//验证地址
                            message: '条码已存在',//提示消息
...
//                    validators: {
//                        remote: {//ajax验证。server result:{"valid",true or false} 向服务发送当前input name值，获得一个json数据。例表示正确：{"valid",true}
//                            url: basePath + "/wms/pl/checkSales.do",  //验证地址
//                            message: '上级为非卖场',//提示消息
//                            delay: 2000,//每输入一个字符，就发ajax请求，服务器压力还是太大，设置2秒发送一次ajax（默认输入一个字符，提交一次，服务器压力太大）
//                            type: 'POST',//请求方式
//                            data: function (validator) {
...
//                },
    ...
    function save(){
        $('#rackEditForm').data('bootstrapValidator').validate();
        if(!$('#rackEditForm').data('bootstrapValidator').isValid()){
            return ;
        }
        saveImg($("#form_barcode").val());
        var formData= $("#rackEditForm").serialize();

        $.ajax({
            type: "POST",
            url: basePath + "/wms/pl/saveRa.do",
            data: formData,
            dataType: "json",
    ...
    function saveImg(barcode){
        ...
        xhr.open("POST", basePath + "/wms/pl/saveImg.do?barcode="+barcode+"&flag=ra",false);
        xhr.send(fd);
    }
</script>
