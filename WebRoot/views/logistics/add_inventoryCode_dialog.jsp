...
<script>
    ...
    $(function () {
       $("#add-inventory-dialog").on('show.bs.modal', function () {
        ...
        $("#add_uniqueCodes").keydown(function (event) {
            ...
            if (event.keyCode == 13) {
               ...
               /* if (taskType === -1) {
                ...
                ajax_url = basePath + "/logistics/inventoryBillController/checkconsignmentEpcStock.do";
                ajax_data = { code: code};
                $.ajax({
                    async: false,
                    url: ajax_url,
                    ...
                    success: function (data) {
                        if (data.success) {
                            ...
                            $.each($("#addDetailgrid").getDataIDs(), function (index, value) {
                                var Detailgrid=$("#addDetailgrid").getRowData(value);
                                if(Detailgrid.sku==data.result.sku){
                                    isok=false;
                                    var inQty=Detailgrid.inQty;
                                    var outQty=Detailgrid.outQty;
                                    var outMonyQty=Detailgrid.outMonyQty;
                                    var sum=parseInt(inQty)-parseInt(outQty)-parseInt(outMonyQty);
                                    var num=0;
                                    $.each($("#inventoryCodeGrid").getDataIDs(), function (index, value) {

                                        var inventoryCodegrid=$("#inventoryCodeGrid").getRowData(value);
                                        if(inventoryCodegrid.sku==data.result.sku){
                                            num=num+1;
                                        }
                                    });
                                    if(parseInt(sum)-num<=0){
                                        $.gritter.add({
                                            text: data.result.sku+'已超过能退货的数量',
                                            class_name: 'gritter-success  gritter-light'
                                        });
                                    }else{
                                        $("#inventoryCodeGrid").addRowData($('#inventoryCodeGrid').getDataIDs().length, data.result, 'first');
