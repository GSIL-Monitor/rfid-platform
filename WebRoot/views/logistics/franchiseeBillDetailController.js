...
function initcodeDetail() {
    ...
    $("#codeDetailgrid").jqGrid('setGridWidth', parent_column.width() - 5);
}
function initGrid() {
    $("#addDetailgrid").jqGrid({
        ...
        colModel: [
            ...
            {
                name: 'tagPrice', label: '销售价格', width: 40,
                editrules: {
                    number: true
                }
            },
...
function saveItem(rowId) {
    ...
    var value = $('#addDetailgrid').getRowData(rowId);
    /* if((parseInt(value.qty)-parseInt(value.outQty))>=parseInt(value.returnQty)){*/
...
function edit_wareHouseOut() {
    ...
    } else if (sum_qty > sum_outQty + sum_returnQty) {
        skuQty = {};
        $.each($("#addDetailgrid").getDataIDs(), function (index, value) {
            var rowData = $("#addDetailgrid").getRowData(value);
