...
/**
 * Created by session on 2017/3/25 0025.
 */

@Controller
@RequestMapping(value = "/hall/forceInbound")
public class HallForceInboundController extends BaseController {
	...
	@Override
	public String index() {
		return "/views/hall/forceInbound";
	}
	...
	DataSourceResult read(@RequestBody DataSourceRequest request) {
		...
		if (request.getGroup().size() == 0) {
			for (HallTaskDetailView htdv : (List<HallTaskDetailView>) dataSourceResult.getData()) {
				...
				if(CommonUtil.isNotBlank(htdv.getCustomerId())){
