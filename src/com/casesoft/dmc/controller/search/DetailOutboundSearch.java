...
@Controller
@RequestMapping("/search/detailOutboundViewSearch")
public class DetailOutboundSearch {
	
	@Autowired
	DetailOutboundDao detailOutboundDao;
	@RequestMapping(value = "/index")
    public String index(){
    	return "/views/search/detailOutboundViewSearch";
    }
    
    @RequestMapping(value = "/list", method = RequestMethod.POST)
    public @ResponseBody DataSourceResult read(@RequestBody DataSourceRequest request) {

        DataSourceResult dataResult = detailOutboundDao.getList(request);

        for(DetailOutboundView dtl :(List<DetailOutboundView>) dataResult.getData()){
