...
public class AgentController extends BaseController implements IBaseInfoController<Unit> {
	...
	public MessageBox save(Unit unit) throws Exception {
		...
		if (CommonUtil.isBlank(u)) {
			...
			if (CommonUtil.isBlank(unit.getCode())) {
				String code = this.agentService.findMaxCode(Constant.UnitType.Agent);
				u.setCode(code);
			} else {
		...
		u.setType(Constant.UnitType.Agent);
