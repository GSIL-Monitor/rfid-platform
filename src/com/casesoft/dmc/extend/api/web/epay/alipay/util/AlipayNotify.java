...
public class AlipayNotify {
  ...
  /**
   * result=0: 支付宝支付失败1：支付宝支付成功，本地更新单据失败2：都成功
   * 
   * @param billNo
   * @param params
   * @return
   */
  public static int processBillDBStatus(String billNo, Map<String, String> params) {
    int status = 0;
    if (!verify(params)) {
      return status;
    }
    ...
    try {
      BillService billService = (BillService) SpringContextUtil.getBean("billService");
      Bill bill = billService.get("billNo", billNo);
      bill.setPayState(2);
      bill.setPayTime(CommonUtil.getDateString(new Date(), "yyyy-MM-dd HH:mm:ss"));
