...
public class MetersBonWeTag extends AbstractBaseTag {

	public MetersBonWeTag() {
		super();
	}

	public MetersBonWeTag(String styleId, String colorId, String sizeId) {
		super();
	...
	public String getEpc() {
		...
		/*
		 * String sku = this.styleId + this.colorId + this.sizeId; Product p =
		 * CacheManager.getProductByCode(sku); String SerialNo =
		 * this.uniqueCode.
		 * substring(this.uniqueCode.length()-7,this.uniqueCode.length()-2);
	...
	public String getUniqueCode(int startNo, int i) {
		...
		this.uniqueCode = p.getId()
				+ CommonUtil.produceIntToString(startNo + i - 1,
						this.getSerialLength()) + Constant.TagSerial.MetersBonWe;
		return uniqueCode;
	}
