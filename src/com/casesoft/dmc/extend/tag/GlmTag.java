...
/**
 * Created by WinLi on 2017-05-26.
 */
public class GlmTag extends AbstractBaseTag {
    public GlmTag() {
        super();
    }

    public GlmTag(String styleId, String colorId, String sizeId) {
        super();
    ...
    public String getEpc() {
        int length = this.uniqueCode.length();
        String seqNo = this.uniqueCode.substring(length-4,length);
        String sku = this.uniqueCode.substring(0,length-4);
        Product p = CacheManager.getProductByCode(sku);
        this.epc = p.getId()+"0"+seqNo+Constant.TagSerial.Glm;
        this.epc = this.epc + CommonUtil.produceIntToString(0, 24 - this.epc.length());
        return this.epc;
    }
    ...
    public String getUniqueCode(int startNo, int i) {
        if(CommonUtil.isBlank(sku)) {
            sku = this.styleId + this.colorId + this.sizeId;
        }
        Product p = CacheManager.getProductByCode(sku);
        this.uniqueCode = p.getCode()
                + CommonUtil.produceIntToString(startNo + i - 1,
                this.getSerialLength()) + Constant.TagSerial.Glm;
        return uniqueCode;
    }
    ...
    @Override
    public boolean isImportUniqueCode() { return true; }
}
