...
import com.casesoft.dmc.extend.playlounge.dao.basic.IPlayloungeShopPostTaskDao;
...
public class PlayloungeShopPostTaskDao extends PlayloungeBasicDao implements
        IPlayloungeShopPostTaskDao {

    @Override
    public void batchShopInTask(final Business bus) {
                ...
                int[] update = this.playloungeJdbcTemplate.batchUpdate(chrsql,
                        new BatchPreparedStatementSetter() {
                            ...
                            public void setValues(PreparedStatement ps, int i)
                                ...
                                ps.setLong(3, dtl.getScanQty().longValue());
    ...
    @Override
    public void batchShopTransferInTask(final Business bus) {
        final Bill bill = bus.getBill();
        if (CommonUtil.isNotBlank(bill)) {
            status = bill.getStatus();
            noticeTable = "dj_zhda";
			 ...
			 * ErpBillIdFactory.Table.WAREH_OUT, bill.getUnitId());
			 */
			/*
			 * 
			 * 更改库存 *
    ...
    @Override
    public void batchShopTransferOutTask(final Business bus) {
        ...
        final String billNo = erpBillIdFactory.productBillId(
                ErpBillIdFactory.Table.SHOP_TRANSFER_OUT, bus.getOrigId());
        String mainSql = String.format(
                "insert %s (accept,item,BillDate,whichprice,"
                ...
                new PreparedStatementSetter() {
                    ...
                    public void setValues(PreparedStatement ps)
                        ...
                        ps.setTimestamp(3, Timestamp.valueOf(CommonUtil
                                .getDateString(new Date(),
                                        "yyyy-MM-dd 00:00:00")));
                        ps.setString(4, bus.getOrigId());
                        ps.setString(5, bus.getDestId());
                        ps.setLong(6, bus.getTotEpc());
                        ps.setDouble(7, bus.getTotPrice());
                        ps.setDouble(8, bus.getTotPreCase());
                        ps.setString(9, "1");// 设置审核flage
                        ps.setString(10, "0");// 设置中止status
                        ps.setString(11, "1");// 设置提交submit
                        ps.setString(12, "0");// 设置执行execution
                        ps.setString(13, (CommonUtil.isBlank(bus.getRemark()) ? "" : bus.getRemark()));
                        if (CommonUtil.isBlank(bus.getOperator())) {
                            ps.setString(14, bus.getDeviceId());
                            ps.setString(16, bus.getDeviceId());
                        } else {
                            ps.setString(14, bus.getOperator());
                            ps.setString(16, bus.getDeviceId());
                        }
                        ps.setTimestamp(15, new Timestamp(new Date().getTime()));
                        ps.setTimestamp(17, new Timestamp(new Date().getTime()));
                    }
                });
    ...
    @Override
    public void batchShopReturnOutTask(final Business bus) {
         ...
         * */
        final String billNo = erpBillIdFactory.productBillId(
                ErpBillIdFactory.Table.SHOP_RETURN_OUT, bus.getOrigId());
        String mainSql = String.format(
                "insert %s (accept,item,BillDate,whichprice,"//4
                        + "Outcode,Incode,types,"//7
                        + "num,mny,cbmny1,"//10
                        + "flag,status,submit,execution,"//14
                        + "prints,printnum,mem,"//17
                        + "gzczy,gzrq,lrczy,lrrq)"//21
                        + " values( ?,?,?,1,"// 4
                        + "?,?,0," //7
                        + "?,?,?,"//10
                        + "?,?,?,?,"// 14
    ...
    @Override
    public void batchShopInventoryTask(final Business bus) {
        mainTable = "dj_zgpdda";
        mainDtlTable = "dj_zgpddb";
        final String billNo = erpBillIdFactory.productBillId(
                ErpBillIdFactory.Table.SHOP_INVENTORY, bus.getOrigId());
